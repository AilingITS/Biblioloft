...
public class user_literaturaAdapter extends RecyclerView.Adapter<user_literaturaAdapter.booksHolder>{

    Context context;
    ArrayList<user_Literatura> list;

    DatabaseReference dbRef;

    public user_literaturaAdapter(Context context, ArrayList<user_Literatura> list) {
        this.context = context;
        this.list = list;
    }

    public user_literaturaAdapter.booksHolder onCreateViewHolder(@NonNull @NotNull ViewGroup parent, int viewType) {
        View v = LayoutInflater.from(context).inflate(R.layout.item_list_book_user,parent,false);
        return new user_literaturaAdapter.booksHolder(v);
    }

    public void onBindViewHolder(@NonNull @NotNull user_literaturaAdapter.booksHolder holder, int position) {

        dbRef = FirebaseDatabase.getInstance().getReference().child("books").child("literatura");

        user_Literatura books = list.get(position);
