...
public class user_misterioAdapter extends RecyclerView.Adapter<user_misterioAdapter.booksHolder>{

    Context context;
    ArrayList<user_Misterio> list;

    DatabaseReference dbRef;

    public user_misterioAdapter(Context context, ArrayList<user_Misterio> list) {
        this.context = context;
        this.list = list;
    }

    public user_misterioAdapter.booksHolder onCreateViewHolder(@NonNull @NotNull ViewGroup parent, int viewType) {
        View v = LayoutInflater.from(context).inflate(R.layout.item_list_book_user,parent,false);
        return new user_misterioAdapter.booksHolder(v);
    }

    public void onBindViewHolder(@NonNull @NotNull user_misterioAdapter.booksHolder holder, int position) {

        dbRef = FirebaseDatabase.getInstance().getReference().child("books").child("misterio");

        user_Misterio books = list.get(position);
