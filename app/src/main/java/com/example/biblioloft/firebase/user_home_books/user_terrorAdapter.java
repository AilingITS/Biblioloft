...
public class user_terrorAdapter extends RecyclerView.Adapter<user_terrorAdapter.booksHolder>{

    Context context;
    ArrayList<user_Terror> list;

    DatabaseReference dbRef;

    public user_terrorAdapter(Context context, ArrayList<user_Terror> list) {
        this.context = context;
        this.list = list;
    }

    public user_terrorAdapter.booksHolder onCreateViewHolder(@NonNull @NotNull ViewGroup parent, int viewType) {
        View v = LayoutInflater.from(context).inflate(R.layout.item_list_book_user,parent,false);
        return new user_terrorAdapter.booksHolder(v);
    }

    public void onBindViewHolder(@NonNull @NotNull user_terrorAdapter.booksHolder holder, int position) {

        dbRef = FirebaseDatabase.getInstance().getReference().child("books").child("terror");

        user_Terror books = list.get(position);
