...
public class user_cientificoAdapter extends RecyclerView.Adapter<user_cientificoAdapter.booksHolder>{

    Context context;
    ArrayList<user_Cientifico> list;

    DatabaseReference dbRef;

    public user_cientificoAdapter(Context context, ArrayList<user_Cientifico> list) {
        this.context = context;
        this.list = list;
    }

    public user_cientificoAdapter.booksHolder onCreateViewHolder(@NonNull @NotNull ViewGroup parent, int viewType) {
        View v = LayoutInflater.from(context).inflate(R.layout.item_list_book_user,parent,false);
        return new user_cientificoAdapter.booksHolder(v);
    }

    public void onBindViewHolder(@NonNull @NotNull user_cientificoAdapter.booksHolder holder, int position) {

        dbRef = FirebaseDatabase.getInstance().getReference().child("books").child("cientifico");

        user_Cientifico books = list.get(position);
